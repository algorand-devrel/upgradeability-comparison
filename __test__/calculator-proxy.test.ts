...
import { ImmutableV4Client } from '../contracts/clients/ImmutableV4Client';
...
let immV4Client: ImmutableV4Client;
...
describe('ProxyPattern', () => {
  ...
  beforeAll(async () => {
    ...
    immV4Client = new ImmutableV4Client(
      {
        sender: testAccount,
        resolveBy: 'id',
        id: 0,
    ...
    await Promise.all(
      [calcPV1Client, calcPV2Client, immV1Client, immV2Client, immV3Client, immV4Client, stoV1Client]
  ...
  test('v3 -> v4', async () => {
    const [calcPV2Reference, immV4Reference, stoV1Reference] = await Promise.all([
      calcPV2Client.appClient.getAppReference(),
      immV4Client.appClient.getAppReference(),
      stoV1Client.appClient.getAppReference(),
    ]);

    await Promise.all([
      calcPV2Client.setTargetApplication({ app: immV4Reference.appId }),
      immV4Client.setSourceApplication({ app: calcPV2Reference.appId }),
      immV4Client.setStorageApplication({ app: stoV1Reference.appId }),
      stoV1Client.setSourceApplication({ app: immV4Reference.appId }),
    ]);

    const correctResult = await calcPV2Client.sumMr(
      { a: 3, targetApp: immV4Reference.appId, storageApp: stoV1Reference.appId },
      { sendParams: { fee: microAlgos(3_000) } },
    );
    expect(correctResult.return).toBe(BigInt(5));
  });
